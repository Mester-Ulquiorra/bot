...
async function showPunishmentById(interaction: ChatInputCommandInteraction | ButtonInteraction, id: string) {
	...
	if (!punishment) return "The punishment wasn't found.";
...
async function createPunishmentsEmbed(userid: string, punishments: Array<DBPunishment>, page: number, max_page: number) {
	// create the embed
	const returnEmbed = CreateEmbed(`**Punishments of <@${userid}> (page ${page} / ${max_page})**`).setFooter({
		text: `User ID: ${userid}`,
	});
		...
		returnEmbed.addFields([
			{
				...
				value:
					...
					`**At** <t:${punishment.at}> **until** ${punishment.until != -1 ? `<t:${punishment.until}>` : `Permanent`} (Active: ${
