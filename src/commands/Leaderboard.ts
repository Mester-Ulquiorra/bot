...
const LeaderboardCommand: SlashCommand = {
    ...
    async run(interaction, client) {
        ...
        if (levels.length === 0) {
            return loc.get(userLang, "error.noLevels");
        }
...
async function ReadFromPage(page: number, maxPage: number, lang: LocLanguage) {
    if (!Cache.has(page)) {
        return loc.get(lang, "error.uncachedPage");
    }
...
async function CachePage(levels: Array<DBLevel>, page: number, client: Client) {
    ...
    for (let j = startIndex; j < startIndex + PageSize && j < levels.length; j++) {
        // read level
...
// let's set up a timer to reset the cache
setInterval(() => {
    if (Cache.size === 0) {
        return;
    }
