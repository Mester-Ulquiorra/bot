...
async function getSteamApp(interaction: ChatInputCommandInteraction) {
    ...
    if (isDLC(DLCs)) {
        ...
        for (let i = 0; i < DLCs.length; i++) {
            ...
            if (dlc) {
                dlcFinal.push(hyperlink(dlc.name + (i + 1).toString(), `https://store.steampowered.com/app/${DLCs[i]}`));
            }
        }
    ...
    if (typeof details.header_image === "string") {
...
async function getUserID(data: string) {
    ...
    let userURL = data;
    if (/^\d+/.test(userURL)) {
        userURL = `https://steamcommunity.com/profiles/${userURL}`;
    } else if (!userURL.startsWith("https")) {
        userURL = `https://steamcommunity.com/id/${userURL}`;
    }
