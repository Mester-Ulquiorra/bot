...
export async function InternalMute(
    ...
    if (!CanPerformPunishment(userConfig, PunishmentType.Mute, duration)) {
        return GetError("InsufficentModLevel");
    }
    if (!CanManageUser(userConfig, targetConfig) || target.user.bot) {
    ...
    const object = punishment.toObject({ versionKey: false });
    delete object._id;
