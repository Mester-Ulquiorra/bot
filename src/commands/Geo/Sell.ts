...
const SellCommand: SlashCommand = {
    ...
    async run(interaction) {
        ...
        if (!IsSellableItem(itemName)) {
            return "That item cannot be sold, yet you somehow tried to sell it, are you a cheater?";
        }
        ...
        if (userItem.count < amount) {
            return "You don't have that many of that item in your inventory.";
        }
        ...
        if (userItem.count === 0) {
            geoConfig.inventory.items.splice(geoConfig.inventory.items.indexOf(userItem), 1);
        }
