...
const TicketCommand: SlashCommand = {
	...
	async runButton(interaction: ButtonInteraction, _client: Client) {
		...
		if (interaction.customId === "ticket.reopen") return reopen(interaction, userConfig);
		if (interaction.customId === "ticket.delete") return deleteTicket(interaction, userConfig);
		if (interaction.customId === "ticket.accept") return accept(interaction, userConfig);
		if (interaction.customId === "ticket.cancelsendto") return cancelSendTo(interaction, userConfig);
...
async function manageUser(
	...
	if (!CanManageUser(userConfig, targetConfig)) return GetError("BadUser");
	...
	if (type === "REMOVE") {
		...
		if (!ticketUsers.has(target.id)) return "That user is not in the ticket.";
	...
	if (type === "ADD") {
		...
		if (ticketUsers.has(target.id)) return "That user is already in the ticket.";
...
async function accept(interaction: ButtonInteraction, userConfig: DBUser) {
	...
	if (userConfig.mod < ticket.waitingfor) return GetError("Permission");
