    ...
    async run(interaction) {
        switch (interaction.options.getSubcommand()) {
            case "play": {
                ...
                if (!userConfig.settings.allowGameInvites) {
                    return "That user has disabled game invites.";
                }
...
class ChessGame {
    ...
    async performTurn(): Promise<void> {
        ...
        try {
            ...
            for (const move of moves) {
                ...
                const pieceToHit = this.chessGame.get(move.to);
                ...
                if (pieceToHit) {
                    emoji = await ChessGame.getPieceEmoji(pieceToHit);
                }
                ...
                if (move.flags.includes("p")) {
                    thisMoveString += ` (Promotion)`;
                }
                if (move.flags.includes("c") || move.flags.includes("e")) {
                    thisMoveString += ` (Capture)`;
                }
                if (move.flags.includes("k") || move.flags.includes("q")) {
                    thisMoveString += ` (Castle)`;
                }

                const promotionText = move.promotion ? ` and promote it to a ${move.promotion}` : "";

                movesOption.push({
                    label: thisMoveString,
                    value: `${move.from}-${move.to}-${move.promotion ?? "none"}`,
                    emoji: finalEmoji,
                    description: `Move your piece to ${move.to} ${promotionText}`
                });
            }
            ...
            const selectionMatch = RegExp(/(.*)-(.*)-(.*)/).exec(selection) as RegExpMatchArray;
            ...
            if (this.chessGame.isGameOver()) {
                if (this.chessGame.isDraw()) {
                    this.end("Lmao, it's a draw", true);
                } else {
    ...
    async end(reason: string, isDraw?: boolean) {
        ...
        if (this.inputMessage) {
            this.inputMessage.delete();
        }

        // stop the interaction collector
        this.componentCollector.stop("The game has ended.");

        if (this.chessGame.isDraw() || !this.winner) {
            this.chessGame.header("Result", "1/2-1/2");
        } else if (this.winner.id === this.player1.id) {
            this.chessGame.header("Result", "1-0");
        } else if (this.winner.id === this.player2.id) {
            this.chessGame.header("Result", "0-1");
        }
    ...
    static getGameByPlayer(id: string): { game: ChessGame; player: 1 | 2 } | null {
        if (ChessGame.ActiveGames.size === 0) {
            return null;
        }

        for (const [, game] of ChessGame.ActiveGames) {
    ...
    static async getPieceEmoji(piece: chess.Piece) {
        ...
        if (rawEmoji === "❓") {
            ...
            return await GetGuild().emojis.fetch(rawEmoji);
        }
    }
}
