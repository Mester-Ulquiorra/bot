...
const SetModCommand: SlashCommand = {
	...
	async run(interaction: ChatInputCommandInteraction, _client: Client) {
		...
		if (!CanManageUser(userConfig, targetConfig) || userConfig.mod < ModNameToLevel("Head")) return GetError("BadUser");

		if (modLevel >= userConfig.mod) return GetError("BadValue", "modlevel");
