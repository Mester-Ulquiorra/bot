...
import { CanManageUser, ModNameToId, ModNameToLevel, ModType } from "../util/ModUtils";
...
const SetModCommand: SlashCommand = {
	...
	async run(interaction: ChatInputCommandInteraction, _client: Client) {
		...
		Log(`${target.user.tag} (${target.id}) has been set to mod level ${modLevel} by ${interaction.user.tag} (${interaction.user.id})`);

		const returnEmbed = CreateEmbed(
			`**${target} has been set to mod level ${modLevel}**`,
			{ color: EmbedColor.Success }
		).addFields([{ name: "Reason", value: reason, inline: true }]);

		const userEmbed = CreateEmbed(
			`**You have been set to mod level ${modLevel} by ${interaction.user}**`,
			{ color: EmbedColor.Success }
		).addFields([{ name: "Reason", value: reason, inline: true }]);
