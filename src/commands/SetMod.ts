...
export async function SetModRole(member: GuildMember, modLevel: number) {
    ...
    if (modLevel !== ModNameToLevel("Head")) {
        ManageRole(member, ModNameToId("Head"), "Remove");
    }
    if (modLevel !== ModNameToLevel("Test")) {
        ManageRole(member, ModNameToId("Test"), "Remove");
    }
    if (modLevel <= 0 || modLevel >= ModNameToLevel("Head")) {
        ManageRole(member, ModNameToId("Base"), "Remove");
    }

    // go through basemod names and remove all roles
    for (const modName of ["Level 1", "Level 2", "Level 3"] as ModName[]) {
        if (modLevel !== ModNameToLevel(modName)) {
            await ManageRole(member, ModNameToId(modName), "Remove");
        }
    }
