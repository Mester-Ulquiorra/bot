...
class TriviaGame {
     ...
     */
    constructor(message: Message, player: GuildMember, category: number, difficulty: TriviaDifficulty = "medium") {
        this.player = player;
        ...
        this.questions = [];
        this.summaryEmbed = CreateEmbed("This in an error, you should not see this!");
            ...
            embed.addFields([
                {
                    ...
                    value: `Correct answer: ${question.correctAnswer} | Player's answer: ${question.userAnswer ?? "ran out of time"}`
                }
            ]);
        }
        ...
        this.message
            ...
            .then(async (interaction) => {
                ...
                if (this.questions[this.turn].checkAnswer(userAnswer)) {
                    ...
                    // incorrect answer
                    let correctAnswer = this.questions[this.turn].correctAnswer;

                    if (this.questions[this.turn].type === "boolean") {
                        correctAnswer = correctAnswer === "True" ? "Correct" : "Incorrect";
                    }
     ...
     */
        ...
        if (didQuit) {
            const embed = CreateEmbed(reason ?? "You quit the game!", {
                title: `Trivia game (Question ${this.turn + 1}/${this.questions.length})`,
                color: "error"
            });
            this.message.edit({ embeds: [embed], components: [] });
