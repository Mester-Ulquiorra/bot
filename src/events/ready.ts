...
async function SetupVerifyListener() {
	...
	const verifyMessage = (await verifyChannel.messages.fetch()).filter((m) => m.author.id === Ulquiorra.user.id).last();
	...
	verifyMessage
		...
		.on("collect", async (interaction) => {
			...
			const cooldown = verifyCooldown.get(interaction.user.id) ?? verifyCooldown.set(interaction.user.id, 0).get(interaction.user.id);

			// check for cooldown
			if (cooldown > Date.now() && !testMode) {
				// they're in cooldown
				interaction.followUp({
					content: "You are currently in cooldown, please try again later.",
					ephemeral: true,
				});
				return;
			}
			...
			const messages = await botMessage.channel
				...
				.catch(() => {
					...
					botMessage.edit({
						embeds: [
							CreateEmbed(`You didn't answer in time.`, {
								color: "error",
							}),
						],
					});
			...
			if (userMessage.content === captcha.answer.toString()) {
				...
				ManageRole(interaction.member, config.roles.Member, "Add", "verified user");

				ManageRole(interaction.member, config.roles.Unverified, "Remove", "verified user");

				// edit the message
				botMessage.edit({
					embeds: [
						CreateEmbed(`You have been successfully verified!`, {
							color: "success",
						}),
					],
				});
				...
				botMessage.edit({
					embeds: [
						CreateEmbed(`The answer you entered was incorrect.\nThe correct answer was: ${captcha.answer}`, { color: "error" }),
					],
				});
			}
		});
