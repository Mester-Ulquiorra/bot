...
const GuildMemberUpdateEvent: Event = {
	...
	async run(_client, oldMember: GuildMember, newMember: GuildMember) {
		if (oldMember.roles.cache.size !== newMember.roles.cache.size) roleChange(oldMember, newMember);

		if (oldMember.nickname !== newMember.nickname) nicknameChange(oldMember, newMember);
	},
};
...
async function roleChange(oldMember: GuildMember, newMember: GuildMember) {
	...
	const difference = oldRoles.filter((role) => !newRoles.includes(role)).concat(newRoles.filter((role) => !oldRoles.includes(role)));
...
async function nicknameChange(oldMember: GuildMember, newMember: GuildMember) {
	const embed = CreateEmbed(`Nickname of ${newMember} has been changed`).addFields(
		{
			name: "Old nickname",
